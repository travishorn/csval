...
test("validates a valid CSV string with rules", async () => {
  const parsed = await parseCsv("name,age\nJohn,30");
...
test("validates a valid CSV string with a boolean", async () => {
...
test('validates rules with the optional type:"object" property', async () => {
  const parsed = await parseCsv("name,age\nJohn,30");
...
test("throws an error when extra properties are specified while additionalProperties is set to false", async () => {
  const parsed = await parseCsv("name,age,salary\nJohn,10,5000");
